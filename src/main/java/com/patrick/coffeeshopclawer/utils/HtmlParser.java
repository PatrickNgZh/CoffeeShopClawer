    ...
    public List<CoffeeShop> parseWebPage(String url) throws IOException {
        ...
        return shops.stream()
                .map(e -> new CoffeeShop(e.select(".addr").text(),
                        ...
                        e.select(".comment-list span b").text().split(" ")[0],//problem of String index out of range: -1
                        e.select(".comment-list span b").text().split(" ")[1],//because == compare with address
                        e.select(".comment-list span b").text().split(" ")[2],//and equals() compare with values
